...
ggplot(spl_data, aes(x = center_time, y = spl_low_db))+ geom_point(alpha = .2)+
  geom_line(aes(y = predict(circ_interactmod_centered)))+ facet_grid(Site~Deploy, scales = "free_x")
...
#circ_interactmod <- gls(spl_low_db ~ center_time+ (sin(2*pi*center_time/24) + cos(2*pi*center_time/24))*id,
#                        correlation = corCAR1(~center_time|Site*Deploy), data=spl_data)
