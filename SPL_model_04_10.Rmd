...
```{r}
cyclic_gam = gam(spl_low_db ~ s(hrmn_edt, bs="cc", k=96,by = Deploy)+ Deploy/factor(day) + Site*Deploy, data=spl_data)
#cyclic_gam = gam(spl_low_db ~ s(hrmn_edt, bs="cc", k=96,by = Deploy)+ s(center_time, by = interaction(Site,Deploy)) + Site*Deploy, data=spl_data)
ggplot(spl_data, aes(x = center_time, y = spl_low_db))+ geom_point()+
  geom_line(aes(y = predict(cyclic_gam)))+facet_grid(Site~Deploy)
